...
import { renderTransactionId } from "../../../lib";
...
export const useNotification = () => {
  ...
  const showNotification = (render: string | ReactNode, type: TypeOptions) => {
    if (notification.current) {
      ...
      notification.current = type === "default" ? toast.info(render, { onClose }) : toast[type](render, { onClose });
    }
  };
  ...
  return {
    ...
    transactionSubmitNotification: (transactionId: string) =>
      showNotification(<span>Transaction submitted: {renderTransactionId(transactionId)}</span>, "info"),
    transactionSuccessNotification: (transactionId: string) =>
      showNotification(<span>Transaction successful: {renderTransactionId(transactionId)}</span>, "success"),
  };
};
